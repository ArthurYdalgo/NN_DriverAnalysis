...
class MyGame(arcade.Window):
    ...
    def reset_log(self):              
        F = open("Logs/Rede4_4-24-3/R4_"+str(amostra)+".txt","a")                                 
        F.write(str(int(self.distance_car))+"\n")     
        if(self.resetCount>=120):
            storage.save(nn, filepath=f'Saves/Rede4_4-24-3/motorista{amostra}_{CODINOME}_g{self.resetCount}_d{self.maxDistance:.0f}.hdf5')
            sys.exit()     
    ...
    def update(self, delta_time):
        ...
        if(self.resetCount>=self.save):                    
            storage.save(nn, filepath=f'Saves/Rede4_4-24-3/motorista{amostra}_{CODINOME}_g{self.save}_d{self.maxDistance:.0f}.hdf5')
            self.save+= SAVE_SPAN
        '''
