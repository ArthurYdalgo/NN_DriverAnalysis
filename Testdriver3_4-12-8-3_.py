...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        for filename in os.listdir('Saves/Rede3_4-12-8-3/'):                
            if filename.startswith('SavedDriver_'+CODINOME+'_'):                                
                ...
                print(dados[1])
                save = "Saves/Rede3_4-12-8-3/"+filename
                #storage.load(nn, filepath=save)
                print("Rede importada: "+save)        
    ...
    def reset_log(self):                        
        ...
        if(self.resetCount>=120):
            #storage.save(nn, filepath=f'Saves/Rede3_4-12-8-3/motorista{amostra}_{CODINOME}_g{self.resetCount}_d{self.maxDistance:.0f}.hdf5')
            sys.exit()   
    ...
    def update(self, delta_time):
        ...
        if(self.resetCount>=self.save):                    
            storage.save(nn, filepath=f'Saves/Rede3_4-12-8-3/motorista{amostra}_{CODINOME}_g{self.save}_d{self.maxDistance:.0f}.hdf5')
            self.save+= SAVE_SPAN
        '''
